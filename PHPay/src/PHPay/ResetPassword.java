...
public class ResetPassword extends JFrame {
	...
	private JLabel wrongLabel, newPassLabel;
	private RoundedPasswordField oldPassField;
	...
	private void initialize(String id) {
		...
		Home.inactive.setRepeats(false);
				...
				blurPanel.setBounds(0, 0, 428, 627);
		...
		mainPane.setBounds(0, 0, 428, 627);
		...
		pass1.setBounds(37, 208, 353, 388);
		...
		newPassLabel = new JLabel("Passwords must match");
		newPassLabel.setVisible(false);

		JLabel oldPassEye = new JLabel("");
		oldPassEye.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (def1) {
					oldPassEye.setIcon(see);
					oldPassField.setEchoChar((char) 0);
					oldPassEye.setBounds(289, 71, 25, 25);
				} else {
					oldPassEye.setIcon(blind);
					oldPassField.setEchoChar('•');
					oldPassEye.setBounds(289, 69, 25, 25);
				}
				def1 = !def1;
			}
		});
		oldPassEye.setBounds(289, 69, 25, 25);
		oldPassEye.setIcon(blind);
		pass1.add(oldPassEye);

		oldPassField = new PHPay.RoundedPasswordField(10);
		oldPassField.setMargin(new Insets(2, 7, 2, 7));
		oldPassField.setForeground(Color.WHITE);
		oldPassField.setFont(new Font("Segoe UI Semibold", Font.PLAIN, 15));
		oldPassField.setColumns(10);
		oldPassField.setBounds(31, 64, 291, 34);
		pass1.add(oldPassField);
		newPassLabel.setHorizontalAlignment(SwingConstants.RIGHT);
		newPassLabel.setForeground(new Color(255, 0, 0));
		newPassLabel.setFont(new Font("Segoe UI Semibold", Font.BOLD, 13));
		newPassLabel.setBounds(138, 134, 183, 23);
		pass1.add(newPassLabel);

		JLabel newEye = new JLabel("");
		newEye.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		newEye.setIcon(blind);
		newEye.setBounds(289, 171, 25, 25);
		pass1.add(newEye);
		newEye.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (def1) {
					...
					newEye.setBounds(289, 173, 25, 25);
				} else {
					newEye.setIcon(blind);
		...
		confirmeEye.setBounds(289, 262, 25, 25);
		...
		confirmeEye.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (def2) {
					...
					confirmeEye.setBounds(289, 264, 25, 25);
				} else {
					confirmeEye.setIcon(blind);
		...
		newPasswordField.setBounds(31, 166, 291, 34);
		...
		confirmPasswordField.setBounds(31, 257, 291, 34);
		...
		lblWe_1_4.setBounds(31, 133, 113, 23);
		pass1.add(lblWe_1_4);
		...
		lblWe_1_4_1.setBounds(31, 225, 219, 23);
		...
		next.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				boolean oldEdited = false, newEdited = false, confirmEdited = false;
				...
				String oldPassHash = AccountVerify.passwordHash(oldPass);


				if (oldPassHash.equals(getPassHash(id))) {
				...
				if (newPass.isEmpty()) {
					newPassLabel.setText("Password invalid");
				...
				if (newPass.equals(confirmPass)) {
					...
					newPassLabel.setText("Passwords should match");
					newPassLabel.setVisible(true);
					newEdited = false;
					confirmEdited = false;
				}
				...
				if (oldEdited && newEdited && confirmEdited) {
					String newPassHash = AccountVerify.passwordHash(confirmPass);
					updatePass(newPassHash, id);
		...
		cancel.setBounds(31, 321, 67, 23);
		pass1.add(cancel);

		JLabel lblWe_1_4_2 = new JLabel("Old Password");
		lblWe_1_4_2.setHorizontalAlignment(SwingConstants.LEFT);
		lblWe_1_4_2.setForeground(Color.LIGHT_GRAY);
		lblWe_1_4_2.setFont(new Font("Segoe UI Semibold", Font.BOLD, 13));
		lblWe_1_4_2.setBounds(31, 31, 113, 23);
		pass1.add(lblWe_1_4_2);

		wrongLabel = new JLabel("Wrong password");
		wrongLabel.setVisible(false);
		wrongLabel.setHorizontalAlignment(SwingConstants.RIGHT);
		wrongLabel.setForeground(new Color(255, 0, 0));
		wrongLabel.setFont(new Font("Segoe UI Semibold", Font.BOLD, 13));
		wrongLabel.setBounds(174, 31, 140, 23);
		pass1.add(wrongLabel);
	...
	public static String getPassHash(String ID) {
		...
		String query = "SELECT `hashed_pass` FROM `users` WHERE user_id = ?";
		String hash = "";
		try {
	...
	public static void updatePass(String newPass, String id) {
		int idInt = Integer.parseInt(id);

		String query = "UPDATE `users` SET `hashed_pass` = ? WHERE user_id = ?";
		try {
